...
function CategoryPost(props) {
    ...
    useEffect(() => {
        ...
        if (props.category !== sessionStorage.cat) {
            if (sessionStorage.getItem(props.category) === null) {
                const onLoad = async () => {
                    const res = await fetch(`/api/getCategoryPost?category=${props.category}`, {
                        method: "GET",
                    });
                    if (res.status === 200) {
    ...
    return (
        ...
        <List
            ...
            dataSource={boardList? boardList.map((c, index) => {
...
const CategoryPostData = (props) => {
    return (
        <li className="years_box oldhitL" >
            ...
            <a href={props.link} className="link_thumb main_log" section_code="old_hit">

            <img src={
                props.rank === 1 ? ramen : props.rank === 2 ? ansung : props.rank === 3 ? jin : shin
            }width="171" height="171"/>

                <img src={
                    props.rank === 1 ? gold : props.rank === 2 ? silver : props.rank === 3 ? bronze : blank
                } className="sp_years icon_years7" />


                <div className="txt_box">
                    <strong className="tit">{props.title}</strong><br></br>
                    <span className="writer">{props.writer}</span><br></br>
                    <span className="gall_name">{props.create}</span>
                    <span className="info_num"><b>좋아요</b> {props.hits}개 <b>조회수</b> {0}회 </span>
                </div>
